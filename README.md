...
- **予測表示**：`予測の時間` を 30 分にすると、30 分後の予測フレームを表示します。
  0 なら常に最新の実況です。
- **深いズーム**：ズーム 4〜16。提供元が配信していないズームでは、地図は指定した
  ズームで描画し、レーダーは提供元の最大ズーム（気象庁は 10、RainViewer は 12）の
  タイルを拡大して重ねます。
- **インタラクティブビューア**（任意）：ドラッグとピンチで移動・拡大できるページを
  配信します。`iframe` カードで埋め込めます。
...
| ズームレベル | 4〜16。大きいほど狭い範囲を詳細に表示します |
...
| `source` | 提供元の名称 |
| `zoom` | 地図を描画したズームレベル |
| `radar_zoom` | レーダータイルを取得したズームレベル |
| `viewer_url` | ビューアのアドレス（有効な場合のみ） |

### ズームについて

地図のズームはベースマップが配信している範囲へ丸められます（地理院タイル淡色は 18、
白地図は 14、OpenStreetMap と CARTO は 19 まで）。

レーダータイルの配信は提供元ごとに上限があり、**気象庁は 10、RainViewer は 12** です。
それより深いズームを指定した場合、地図は指定どおりの縮尺で描画し、レーダーは上限
ズームのタイルを拡大して重ねます。`zoom` と `radar_zoom` が異なるのはこの状態です。

拡大は最近傍法で行うため、雨量の色は元の階調のまま保たれます（中間色を作りません）。
言い換えると、`radar_zoom` を超えたズームでは地図は精細になりますが、雨雲の解像度
そのものは増えません。気象庁の高解像度ナウキャストは 250m メッシュで、ズーム 10 で
ほぼ元の解像度です。

## インタラクティブビューア（ピンチズーム）

Home Assistant の標準カードは画像をピンチで拡大できません。この統合は代わりに、
ドラッグとピンチで操作できる小さなページを配信できます。ジェスチャーが終わるたびに
新しい中心とズームで再描画するため、**拡大すると実際に地図が精細になります**。

### 有効化

1. 設定 → デバイスとサービス → Rain Cloud Radar → **設定** →
   「インタラクティブビューアを有効にする」をオン
2. 開発者ツール → 状態 で `camera.<名前>_レーダー` の属性 `viewer_url` を確認
   （`/api/raincloudradar/<32文字のトークン>` の形式です）
3. ダッシュボードに `iframe` カードを追加

```yaml
type: iframe
url: /api/raincloudradar/ここにトークン
aspect_ratio: 100%
```
...
| ドラッグ | 移動 |
| ピンチイン／アウト | 縮小／拡大 |
| ホイール、ダブルクリック | 拡大・縮小（PC） |
| ＋ / − / ⟲ ボタン | 拡大・縮小・初期位置に戻す |

新しいフレームは更新間隔ごとに自動で取り込まれます。

### セキュリティ上の注意

このページは `<iframe>` から開く必要があるため、**Home Assistant のログインを経由せずに
アクセスできます**。保護は URL に含まれる 32 文字のランダムなトークンだけで、これは
Home Assistant の Webhook と同じ考え方です。

- トークンを知っている人は、あなたの設定地点周辺のレーダー画像を取得できます
  （＝おおよその所在地が分かります）。
- 既定では無効です。必要なときだけ有効にしてください。
- 無効に戻すとエンドポイントは 404 を返します。
- トークンを変更したい場合は、統合を削除して追加し直してください。
